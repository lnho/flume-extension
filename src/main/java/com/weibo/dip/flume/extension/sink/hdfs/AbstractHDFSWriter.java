...
public abstract class AbstractHDFSWriter implements HDFSWriter {
	...
	/**
	 * This method gets the datanode replication count for the current open
	 * file.
	 *
	 ...
	 *
	 * <p/>
	 * If this function returns -1, it means you are not properly running with
	 * the HDFS-826 patch.
	 * 
		...
		if (os != null) {
			...
			try {
				...
				logger.info("Doesn't have access to getNumCurrentReplicas on "
						+ "FileSystems's output stream --HDFS-826 not available; fsOut=" + wrappedStreamClass.getName(),
						e);

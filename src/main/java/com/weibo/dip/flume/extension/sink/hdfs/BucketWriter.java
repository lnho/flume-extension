...
import com.weibo.dip.flume.extension.sink.hdfs.MultithreadingHDFSEventSink.WriterCallback;

/**
 * Internal API intended for HDFSSink use. This class does file rolling and
 * handles file formats and serialization. Only the public methods in this class
 * are thread safe.
 */
class BucketWriter {
	...
	private final int maxConsecUnderReplRotations = 30; // make this
														// config'able?
	...
	// flag that the bucket writer was closed due to idling and thus shouldn't
	// be
	// reopened. Not ideal, but avoids internals of owners
	protected boolean closed = false;
	AtomicInteger renameTries = new AtomicInteger(0);

	BucketWriter(long rollInterval, long rollSize, long rollCount, long batchSize, Context context, String filePath,
			String fileName, String inUsePrefix, String inUseSuffix, String fileSuffix, CompressionCodec codeC,
	...
	private Method getRefIsClosed() {
		try {
			return fileSystem.getClass().getMethod("isFileClosed", Path.class);
		} catch (Exception e) {
			LOG.warn("isFileClosed is not available in the " + "version of HDFS being used. Flume will not "
					+ "attempt to close files if the close fails on " + "the first attempt", e);
			return null;
		}
	}
			...
			try {
				...
				callWithTimeout(new CallRunner<Void>() {
					...
					public Void call() throws Exception {
						if (codeC == null) {
							// Need to get reference to FS using above config
							// before underlying
		...
		if (rollInterval > 0) {
			Callable<Void> action = new Callable<Void>() {
				public Void call() throws Exception {
					...
					try {
						// Roll the file and remove reference from sfWriters
						// map.
						close(true);
					} catch (Throwable t) {
		...
		return new Callable<Void>() {
			...
			public Void call() throws Exception {
				if (renameTries >= maxRenameTries) {
					LOG.warn("Unsuccessfully attempted to rename " + path + " " + maxRenameTries
							+ " times. File may still be open.");
					return null;
				}
				renameTries++;
				try {
					...
					LOG.warn("Renaming file: " + path + " failed. Will " + "retry again in " + retryInterval
							+ " seconds.", e);
	...
	public synchronized void close(boolean callCloseCallback) throws IOException, InterruptedException {
		...
		// NOTE: timed rolls go through this codepath as well as other roll
		// types
		...
		if (bucketPath != null && fileSystem != null) {
			...
			try {
				...
				LOG.warn("failed to rename() file (" + bucketPath + "). Exception follows.", e);
				sinkCounter.incrementConnectionFailedCount();
	...
	public synchronized void flush() throws IOException, InterruptedException {
		...
		if (!isBatchComplete()) {
			...
			if (idleTimeout > 0) {
				// if the future exists and couldn't be cancelled, that would
				// mean it has already run
				// or been cancelled
		...
		if (idleFuture != null) {
			...
			if (!idleFuture.isDone()) {
				try {
					...
					LOG.warn(
							"Timeout while trying to cancel closing of idle file. Idle" + " file close may have failed",
							ex);
				} catch (Exception ex) {
	...
	private <T> T callWithTimeout(final CallRunner<T> callRunner) throws IOException, InterruptedException {
		...
		try {
			...
			future.cancel(true);
			sinkCounter.incrementConnectionFailedCount();
			throw new IOException("Callable timed out after " + callTimeout + " ms" + " on file: " + bucketPath, eT);

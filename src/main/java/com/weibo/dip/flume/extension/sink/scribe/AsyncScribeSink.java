 ...
 * under the License.
 */
package com.weibo.dip.flume.extension.sink.scribe;
...
/**
 * Asynchronous Sink that forwards messages to a scribe listener.
 * <p>
 * The use case for this sink is to maintain backward compatibility with scribe
 * consumers when there is a desire to migrate from Scribe middleware to
 * FlumeNG.
 */
public class AsyncScribeSink extends AbstractSink implements Configurable {
	...
	public Status process() throws EventDeliveryException {
		...
		final AsyncCallBack<Scribe.AsyncClient.Log_call> cb = new AsyncCallBack(lock, condition, callbacksReceived,
				txnFail);
		...
		/*
		 * At this point, either the txn has failed or all callbacks received
		 * and txn is successful.
		 *
		 ...
		 * it is, it is set from true to true only - false happens only in the
		 * next process call).
		 *
		 */
		if (txnFail.get()) {
